...
def expand_to_batch(x: Array, batch_size: int):
  """Expand unbatched `x` to the specified batch_size`."""
  return jnp.tile(
...
class MultiHeadDotProductAttention(Module):
  ...
  def __call__(self,
    ...
    # 12.12: add prefix to key and value
    if self.prefix is not None:
      key_prefix = self.prefix[0]
      value_prefix = self.prefix[1]
      key = jnp.concatenate([key_prefix, key], axis=-3)  # seq length dimension
      value = jnp.concatenate([value_prefix, value], axis=-3)
